...
namespace Engine {
	namespace utils {
		...
		float BoxPointDistance(glm::vec3 mins, glm::vec3 maxes, glm::vec3 point) {
			//Define the planes that box is constructed from
			glm::vec3 planes[6][4] = {
				glm::vec3(mins.x, mins.y, mins.z), glm::vec3(mins.x, maxes.y, mins.z), glm::vec3(mins.x, maxes.y, maxes.z), glm::vec3(mins.x, mins.y, maxes.z),//LeftFace
				glm::vec3(maxes.x, mins.y, mins.z), glm::vec3(maxes.x, maxes.y, mins.z), glm::vec3(maxes.x, maxes.y, maxes.z), glm::vec3(maxes.x, mins.y, maxes.z),//RightFace
				glm::vec3(mins.x, maxes.y, mins.z), glm::vec3(maxes.x, maxes.y, mins.z), glm::vec3(maxes.x, maxes.y, maxes.z), glm::vec3(mins.x, mins.y, maxes.z),//TopFace
				glm::vec3(mins.x, mins.y, mins.z), glm::vec3(maxes.x, mins.y, mins.z), glm::vec3(maxes.x, mins.y, maxes.z), glm::vec3(mins.x, mins.y, maxes.z), //BottomFace
				glm::vec3(mins.x, mins.y, mins.z), glm::vec3(maxes.x, mins.y, mins.z), glm::vec3(maxes.x, maxes.y, mins.z), glm::vec3(mins.x, maxes.y, mins.z),//BackFace
				glm::vec3(mins.x, mins.y, maxes.z), glm::vec3(maxes.x, mins.y, maxes.z), glm::vec3(maxes.x, maxes.y, maxes.z), glm::vec3(mins.x, maxes.y, mins.z)//FrontFace
			};
			float distance = 0.0f;
			for (int i = 0; i < 6; i++) {//For each plane check the distance
				glm::vec3 vector1 = planes[i][1] - planes[i][0];
				glm::vec3 vector2 = planes[i][2] - planes[i][0];
				glm::vec3 vector = glm::cross(vector1, vector2);
				float D = glm::dot(-vector, planes[i][0]);
				float d = glm::abs(glm::dot(vector, point) + D) / glm::sqrt(vector.x * vector.x + vector.y * vector.y + vector.z * vector.z);
				if (d == 0) {//Point doesn't have distance with plane So checking with the lines of the plane
					d = std::min({
						glm::distance(point, glm::closestPointOnLine(point, planes[i][0], planes[i][1])),
						glm::distance(point, glm::closestPointOnLine(point, planes[i][1], planes[i][2])),
						glm::distance(point, glm::closestPointOnLine(point, planes[i][2], planes[i][3])),
						glm::distance(point, glm::closestPointOnLine(point, planes[i][0], planes[i][3])) });
				}
				if (d <= distance || i == 0) {//We need the smallest distance
					distance = d;
				}
			}
			return distance;
