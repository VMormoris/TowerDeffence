...
namespace Engine {
	...
	*/
	Mesh* Loader::load(const char* filename)
	{
		...
		FILE* pFile;
		folderPath = utils::GetFolderPath(filename);
		...
		mesh->objects.erase(std::remove_if(mesh->objects.begin(), mesh->objects.end(), [](Mesh::Object ob) { return ob.start == ob.end; }), mesh->objects.end());
	...
	// read vertices x,y,z
	inline void Loader::read_vertex(const char* buff)
	{
		glm::vec3 v;
		char* pEnd;
	...
	inline void Loader::read_texcoord(const char* buff)
	{
		glm::vec2 vt;
		char* pEnd;
	...
	// read normals x,y,z
	inline void Loader::read_normal(const char* buff)
	{
		glm::vec3 n;
		char* pEnd;
	...
	void Loader::read_face(const char* buff)
	{
		int step = 0;
	...
	void Loader::read_faceLimited(const char* buff)
	{
		int step = 0;
	...
	glm::ivec3 Loader::read_face_component(const char* buff, int& offset)
	{
		int index = 0;
		int v, vt, vn;
		// v/vt/vn format
	...
	void Loader::generateDataFromFaces()
	{
		...
		{
			glm::vec3 first = shared_vertices[shared_faces[face].vertices.x];
			glm::vec3 second = shared_vertices[shared_faces[face].vertices.y];
			glm::vec3 third = shared_vertices[shared_faces[face].vertices.z];

			//Create minOOB and maxOBB
			{
				mesh->minOBB.x = std::min({ mesh->minOBB.x, first.x, second.x, third.x});
				mesh->minOBB.y = std::min({ mesh->minOBB.y, first.y, second.y, third.y});
				mesh->minOBB.z = std::min({ mesh->minOBB.z, first.z, second.z, third.z});

				mesh->maxOBB.x = std::max({ mesh->maxOBB.x, first.x, second.x, third.x});
				mesh->maxOBB.y = std::max({ mesh->maxOBB.y, first.y, second.y, third.y});
				mesh->maxOBB.z = std::max({ mesh->maxOBB.z, first.z, second.z, third.z});
			}
	...
	void Loader::calculate_flat_normals()
	{
	...
	void Loader::calculate_avg_normals(std::vector<glm::vec3>& shared_vertices, std::vector<glm::vec3>& normals, std::vector<unsigned int>& elements)
	{
	...
	void Loader::calculate_tangents()
	{
		mesh->tangents.clear();
	...
	void Loader::read_usemtl(const char* buff, int& currentMaterialID)
	{
		char temp[1024];
		sscanf(buff, "%s", temp);
		// read the material
	...
	void Loader::read_mtllib(const char* buff, const char* filename)
	{
		char temp[1024];
		sscanf(buff, "%s", temp);
	...
	void Loader::add_new_group(const char* buff, int& currentMaterialID)
	{
		char name[1024];
		sscanf(buff, "%s", name);
	...
	void Loader::parseMTL(const char* filename)
	{
		...
		std::string folder(filename);
		folder = utils::GetFolderPath(folder.c_str());
