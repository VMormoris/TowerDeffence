...
namespace Engine {
	...
	bool GameObject::onEvent(const SDL_Event& event) {
		if (event.type == SDL_KEYDOWN) return onKeyPress(event);
			...
			bool flag = true;
			for (Object3D* obj : objects) {
				glm::vec3 vec1 = glm::rotate(obj->getMinOBB(), angle, rotation_vector);
				glm::vec3 vec2 = glm::rotate(obj->getMaxOBB(), angle, rotation_vector);
				...
				glm::vec3 minOBB = center + (min * scale);
				glm::vec3 maxOBB = center + (max * scale);
